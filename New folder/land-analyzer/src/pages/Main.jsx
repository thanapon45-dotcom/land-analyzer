...
export default function Main({ user }) {
  ...
  const canExportPDF = sub.plan !== "free";
  ...
  const exportPDF = () => {
    if (!canExportPDF) return msg("Export PDF ต้องอัปเกรด Plan ครับ", "err");
  ...
  return (
    <>
      ...
      {showPricing && <Pricing currentPlan={sub.plan} onClose={() => setShowPricing(false)} onSelectPlan={(plan) => { setShowPricing(false); msg(`ระบบชำระเงินสำหรับ ${plan.name} กำลังมาเร็วๆ นี้ครับ 🚀`); }} />}
